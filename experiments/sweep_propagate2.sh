...
MARGINS="${2:?comma-separated propagate taus}"
NPROC="${3:-6}"
...
run_one() {
    ...
    local flag=()
    [[ "$margin" != "0" ]] && flag=(--branchcut-propagate-tau "$margin")
    local t0=$SECONDS
...
for margin in ${MARGINS//,/ }; do
    label="bc_prop2_${margin//./p}"
    mkdir -p "$ROOT/results/runs/$label"
    cat > "$ROOT/results/runs/$label/run.meta" <<EOF
method=branchcut
args=--oracle $ORACLE --branchcut-tau $TAU --branchcut-propagate-tau $margin --query-alpha $ALPHA
