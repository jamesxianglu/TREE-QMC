...
ORACLE="${ORACLE:-sym+cf|maj}"; MAJ_TAU="${MAJ_TAU:-0.60}"; CYCLES="${CYCLES:-8}"; ALPHA="${ALPHA:-0.001}"
MINSUP="${MINSUP:-4}"
...
run_one() {
    ...
    local flag=(--branchcut-cycles "$CYCLES")
    local t0=$SECONDS
    "$BINARY" -i "$DATA/$rep/$gt" --blobsearchonly "$REF/$rep/astral4-rooted.tre" \
        --blob --branchcut-blob --oracle "$ORACLE" --branchcut-tau "$spec,$MAJ_TAU" \
...
export -f run_one; export ROOT BINARY GROUP DATA REF ORACLE MAJ_TAU CYCLES ALPHA MINSUP
for spec in ${SPECS//,/ }; do
    label="bctau_${spec//./p}_h${CYCLES}"; mkdir -p "$ROOT/results/runs/$label"
    printf 'method=branchcut\nargs=--oracle %s --branchcut-tau %s --branchcut-min-support %s (%s)\ngene_trees=per-dataset\nrunner=local\nlabel=%s\n' \
        "$ORACLE" "$spec,$MAJ_TAU" "$MINSUP" "cycles=$CYCLES" "$label" > "$ROOT/results/runs/$label/run.meta"
    echo "=== $label on $GROUP ==="
