#!/usr/bin/env bash
#
# Build TREE-QMC, construct a tree of blobs with 3f1a from a binary refinement,
# and compare its non-trivial splits with the true tree of blobs.
#
# Usage:
#   ./compile_and_test_3f1a.sh DATASET [ALPHA] [BETA] [GENE_TREES] [REFINEMENT]
#
# Examples:
#   ./compile_and_test_3f1a.sh n15/00
#   ./compile_and_test_3f1a.sh n15/00 1e-7 0.95 iqtree_500.nwk my_refinement.nwk
#
# DATASET is relative to data/camus-dataset. If REFINEMENT is omitted, the
# script first estimates a binary species tree with TREE-QMC. Supply the same
# refinement to this and compile_and_test_rowsweep.sh for a paired comparison.
...
TAG="${DATASET//\//_}_alpha${ALPHA}_beta${BETA}"
OUTDIR="$HERE/results/compile_test_3f1a/$TAG"
mkdir -p "$OUTDIR"
...
INFERRED_TOB="$OUTDIR/3f1a_tob.nwk"
echo "Constructing 3f1a ToB: $INFERRED_TOB"
