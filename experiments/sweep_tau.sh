...
GROUP="${1:?group}"; ANCHORS_LIST="${2:?comma-separated track-0 taus}"; NPROC="${3:-7}"
ORACLE="${ORACLE:-t1+cf|maj}"; MAJ_TAU="${MAJ_TAU:-0.60}"; HEAVY="${HEAVY:-4}"; ALPHA="${ALPHA:-0.001}"
...
run_one() {
    ...
    "$BINARY" -i "$DATA/$rep/$gt" --blobsearchonly "$REF/$rep/astral4-rooted.tre" \
        --blob --cornerrow-blob --oracle "$ORACLE" --corner-tau "$na,$MAJ_TAU" \
        --heavy-sampling "$HEAVY" \
...
export -f run_one; export ROOT BINARY GROUP DATA REF ORACLE MAJ_TAU HEAVY ALPHA
for na in ${ANCHORS_LIST//,/ }; do
    label="cr_tau_${na//./p}_h${HEAVY}"; mkdir -p "$ROOT/results/runs/$label"
    cat > "$ROOT/results/runs/$label/run.meta" <<META
method=cornerrow
args=--oracle $ORACLE --corner-tau $na,$MAJ_TAU --heavy-sampling $HEAVY --query-alpha $ALPHA
