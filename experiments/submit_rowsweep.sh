#!/usr/bin/env bash
#
# Submit one SLURM job per dataset to evaluate row-sweep on every dataset with a
# precomputed refinement in one or more leaf-count groups. Each job writes its
# own CSV shard; run collect_rowsweep.sh afterwards to merge them into
# results/rowsweep.csv.
#
# Usage:
#   ./submit_rowsweep.sh [--dry-run] [--force] [--no-build] n15 [n25 ...]
#   ./submit_rowsweep.sh n15,n25
#   ./submit_rowsweep.sh n15 10-19
#
...
LOG_ROOT="$RESULTS_DIR/rowsweep_logs"
BIN_SNAPSHOT_ROOT="$RESULTS_DIR/rowsweep_bin"
SBATCH_SCRIPT="$HERE/run_rowsweep.sbatch"
COMPARE_JL="$HERE/compare_inferred_tob.jl"

DELTA="0.25"
QUERY_ALPHA="0.001"

WORKDIR="$(mktemp -d "${TMPDIR:-/tmp}/rowsweep-submit.XXXXXX")"
cleanup() {
    rm -rf -- "$WORKDIR"
}
trap cleanup EXIT
...
while IFS= read -r group; do
    ...
    while IFS= read -r refinement_dataset_dir; do
        ...
        if [[ "$FORCE" -eq 0 && -s "$output_dir/rowsweep.csv" ]]; then
            skipped="$((skipped + 1))"
            continue
        fi

        mkdir -p "$output_dir" "$LOG_ROOT/$group"
        job_log="$LOG_ROOT/$group/rowsweep_${network_id}_%j.log"

        if [[ "$DRY_RUN" -eq 1 ]]; then
            echo "[dry-run] $dataset ($(basename "$gene_tree_path"))"
        else
            sbatch --job-name="rowsweep_${group}_${network_id}" \
                --output="$job_log" \
                --error="$job_log" \
                "$SBATCH_SCRIPT" \
...
if [[ "$DRY_RUN" -eq 1 ]]; then
    ...
    echo "Collect:  $HERE/collect_rowsweep.sh"
fi